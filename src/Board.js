...
export default class Board extends React.Component {
    ...
    constructor(props) {
        ...
        if (window.location.pathname === "/sublets" || window.location.pathname === "/") {

            var temp = [];
            const fStore = firebase.firestore();
            fStore.collection(this.props.type2).doc(this.props.type).collection("Sublets").orderBy('uniqueId').get().then((collection) => {
                    ...
                    <div>
                        ...
                        <Marker key={entry.id}
                            ...
                            onClick={(event) => window.open(
                                'https://scholarhuts.com/post/' + entry.id, '_blank'
    ...
    changePage(pageNum) {
        ...
        } else if (this.state.page * 5 >= this.state.realStateBox.length && pageNum == 1) {
            alert('No next pages');
        }
        else {
    ...
    componentWillReceiveProps(newProps) {
        ...
        while (j < this.state.stateBox.length) {
            ...
            while (startDate != "none") {
                ...
                const sellerStartYear = parseInt(apt.startDate.toString().substring(0, 4));
                ...
                if (buyerStartYear < sellerStartYear || buyerEndYear > sellerEndYear) {
                    passDate = false;
                    break;
                } else if (buyerStartYear === sellerStartYear) { // Starting year is same. Compare starting months.
                    const sellerStartMonth = parseInt(apt.startDate.toString().substring(5, 7));
                    ...
                    if (buyerStartMonth < sellerStartMonth) {
                        ...
                        const sellerStartDay = parseInt(apt.startDate.toString().substring(8, 10));
                ...
                if (buyerEndYear === sellerEndYear) { // Ending year is same. Compare ending months. 
                    const sellerEndMonth = parseInt(apt.endDate.toString().substring(5, 7));
                    ...
                    if (buyerEndMonth > sellerEndMonth) {
                        ...
                        const sellerEndDay = parseInt(apt.endDate.toString().substring(8, 10));
            ...
            if (gender != "none" && gender != apt.gender && (apt.numBath[0] != 1 || apt.numBeds[0] != 1) && apt.gender != "Both") {
            ...
            // Price filter!!!
            if (startPrice != "none" && (parseInt(apt.charge) < parseInt(startPrice.value) || parseInt(apt.charge) > parseInt(endPrice.value))) {
                j++;
                continue;
            }
            // Size filter
            ...
            if (size != "none" && newProps.sizeFilter[0].toString() != apt.roomates.toString()) {
                j++;
                continue;
            }
            ...
            if (room != "none") {
                if (room == "Yes, I want my own bedroom." && apt.privateBed != "Yes, they get a private bedroom." || room == "No, I don't want my own bedroom." && apt.privateBed == "Yes, they get a private bedroom.") {
                    j++;
                    continue;
                }
            }
            ...
            if (bath != "none") {
                if (bath == "Yes, I want my own bathroom." && apt.privateBath != "Yes, they get a private bathroom." || bath == "No, I don't want my own bathroom." && apt.privateBath == "Yes, they get a private bathroom.") {
                    j++;
                    continue;
                }
            }
            ...
            if (parking != "none") {
                if (parking == "Yes, I need parking." && apt.parking == "No, parking is not an option." || parking == "No, I don't need parking." && apt.parking == "Yes, parking is included in the price.") {
                    j++;
                    continue;
                }
            }
            ...
            if (furnished != "none") {
                if (furnished == "Yes, I need a furnished apartment." && apt.furnished == "No, it will not be mostly furnished." || furnished == "No, I don't need a furnished apartment." && apt.furnished == "Yes, it will be mostly furnished.") {
                    j++;
                    continue;
                }
            }
    ...
    render() {
        ...
        if ((this.state.realStateBox == null || this.state.latLng == null) && (window.location.pathname === "/sublets" || window.location.pathname === "/")) {
            ...
            return (
                ...
                <Router >
                    ...
                    <Route exact={true} path="/apartments" render={() => (
                        <h1>Apartments Coming soon!</h1>
                    )} />
                    <Route exact={true} path="/" render={() => (
                        <div style={{ display: "flex", flexDirection: "column" }}>
                            ...
                            <div className="page-change-button">
                                ...
                                <button id="page1" onClick={() => this.changePage(-1)}>Prev</button>
                                <button id='page2' onClick={() => this.changePage(1)}>Next</button>
                            </div>
                        </div>
                    )} />
                    <Route exact={true} path="/sublets" render={() => (
                        <div style={{ display: "flex", flexDirection: "column" }}>
